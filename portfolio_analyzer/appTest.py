...
def runFirstAnalysis():
    ...
    new_portfolio_sim_input = mcf.configure_monte_carlo(new_portfolio_df, initial_mc_weights, num_simulations, num_trading_days)
    ...
    new_stacked_bars_plot = new_combined_plot_df.hvplot.bar(rot=90, y=['std', 'sharpe', 'weight'], legend = True, stacked=True, title = 'Sharpe Ratios and Standard Deviations for our portfolio stocks')
    ...
    new_return_totals = pd.DataFrame(new_portfolio_sim_returns.iloc[-1, :])
    # %%
    ...
    new_median_initial_plot = new_portfolio_median_outcomes.hvplot(ylabel = 'Median returns of new portfolio', xlabel = 'Days of projection', title = f'Median returns from {num_simulations} simulations over {num_trading_days} trading days for QQQ vs ARKK', label = 'ARKK')
